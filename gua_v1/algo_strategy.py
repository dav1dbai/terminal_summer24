...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def to_attack(self, game_state, structure_damage, damage_to_opponent, points):
        # check if likely to build (@DAVID fine tune)
        # kill if confirmed
        if damage_to_opponent >= game_state.enemy_health: 
            return True
            self.stalled_turns = 0
        RESOURCE_CUTOFF = 10
        if int(game_state.get_resources(1)[1]) >= RESOURCE_CUTOFF and self.stalled_turns < 2:
